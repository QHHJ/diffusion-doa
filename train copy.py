...
# ---------------------------
@th.no_grad()
def eval_val_metrics(model, loader, sqrt_ab, sqrt_1m_ab, steps, device, fp16=False, loss_type="eps", x0_weight=0.5):
    model.eval()
    total, denom = 0.0, 0.0
    for cond, x0, mask in loader:
        ...
        # x0 loss模式下避免极端时间步
        if loss_type == "x0":
            max_t = int(0.95 * steps)
            t = th.randint(0, max_t, (B,), device=device, dtype=th.long)
        else:
        ...
        with th.amp.autocast('cuda', enabled=fp16):
            ...
            # 根据 loss_type 计算损失（与训练一致，添加数值稳定措施）
            if loss_type == "eps":
                mse = ((eps_hat - eps) ** 2) * mask
            elif loss_type == "x0":
                ab_safe = th.clamp(ab, min=1e-4)
                x0_hat = (x_t - abm * eps_hat) / ab_safe
                x0_hat = th.clamp(x0_hat, min=-5.0, max=5.0)
                mse = ((x0_hat - x0) ** 2) * mask
            else:  # hybrid
                mse_eps = ((eps_hat - eps) ** 2) * mask
                ab_safe = th.clamp(ab, min=1e-4)
                x0_hat = (x_t - abm * eps_hat) / ab_safe
                x0_hat = th.clamp(x0_hat, min=-5.0, max=5.0)
                mse_x0 = ((x0_hat - x0) ** 2) * mask
                mse = (1 - x0_weight) * mse_eps + x0_weight * mse_x0
...
def main():
    ...
    ap.add_argument("--model-ch", type=int, default=256, 
    ...
    # ========== 损失函数类型 ==========
    ap.add_argument("--loss-type", type=str, default="x0", 
                    choices=["eps", "x0", "hybrid"],
                    help="损失类型：eps=ε-prediction(稳定), x0=x0-prediction(直接), hybrid=混合(推荐)")
    ap.add_argument("--x0-weight", type=float, default=0.5,
                    help="混合损失中x0的权重(0-1)，推荐0.5-0.7，越大越强调重构准确性")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for cond, x0, mask in train_loader:
            ...
            # 采样 t、噪声 ε，并合成 x_t
            # x0 loss模式下避免极端时间步（t > 0.95*steps时alpha_bar太小会导致数值爆炸）
            if args.loss_type == "x0":
                max_t = int(0.95 * args.steps)  # 限制在前95%的时间步
                t = th.randint(0, max_t, (B,), device=device, dtype=th.long)
            else:
            ...
            with th.amp.autocast('cuda', enabled=args.fp16):
                ...
                if args.loss_type == "eps":
                    ...
                    # x0-prediction：从 eps_hat 计算 x0_hat（添加数值稳定措施）
                    ab_safe = th.clamp(ab, min=1e-4)  # 防止除零
                    x0_hat = (x_t - abm * eps_hat) / ab_safe
                    # 数据归一化后典型范围 [-3, 3]，放宽到 [-5, 5] 避免过度裁剪
                    x0_hat = th.clamp(x0_hat, min=-5.0, max=5.0)
                    mse = ((x0_hat - x0) ** 2) * mask
                else:  # hybrid
                    # 混合损失：同时优化 ε 和 x0（添加数值稳定措施）
                    mse_eps = ((eps_hat - eps) ** 2) * mask
                    ab_safe = th.clamp(ab, min=1e-4)
                    x0_hat = (x_t - abm * eps_hat) / ab_safe
                    x0_hat = th.clamp(x0_hat, min=-5.0, max=5.0)
                    mse_x0 = ((x0_hat - x0) ** 2) * mask
                    mse = (1 - args.x0_weight) * mse_eps + args.x0_weight * mse_x0
        ...
        val_loss, val_rmse = eval_val_metrics(model, val_loader, sqrt_ab, sqrt_1m_ab, args.steps, device, 
                                              fp16=args.fp16, loss_type=args.loss_type, x0_weight=args.x0_weight)
        ...
        if is_best:
            ...
            ckpt = {
                ...
                "ema": [p.cpu() for p in ema.shadow],  # 推断时可优先用 EMA
                "args": vars(args),
        ...
        # 输出epoch总结
        epoch_msg = f"Epoch {epoch}, 训练损失:{train_avg_loss:.4f} 训练RMSE:{train_rmse:.4f} 测试损失: {val_loss:.4f} 测试RMSE: {val_rmse:.4f}"
        print(epoch_msg)
        with open(log_path, "a", encoding="utf-8") as f:
            f.write(epoch_msg + "\n")
